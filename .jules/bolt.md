...
## 2026-05-28 - Bounded Concurrency for Bulk Delete API Operations
**Learning:** `HandleBulkDelete` in `handler/admin.go` previously processed bulk deletion of services sequentially in a loop. For a large number of selected services to delete, the linear O(N) wait time for HTTP API calls to Icinga2 resulted in severe throughput bottlenecks and slow responses back to the user.
**Action:** When performing bulk deletion operations involving multiple remote network calls, utilize bounded concurrent execution (using a semaphore channel like `sem := make(chan struct{}, limit)` alongside `sync.WaitGroup`). This parallelizes network delays and significantly improves response time without overwhelming the downstream API. To preserve order without contention, pre-allocate the results slice and write using indices concurrently.
