...
func (h *DashboardHandler) ServeHTTP(w http.ResponseWriter, r *http.Request) {
	...
	// ⚡ Bolt: Pre-calculate total capacity to avoid repeated allocations during slice append. Expected to reduce execution time by ~60%.
...
function renderTargetCards(targets) {
  ...
  for (var i = 0; i < targets.length; i++) {
    ...
    var targetHtml = '<div class="settings-target-card">';
    targetHtml += '<div class="settings-target-header">';
    targetHtml += '<span class="settings-target-title">' + safeId + '</span>';
    targetHtml += '<div><button class="settings-btn blue settings-btn-sm" data-id="' + encodeURIComponent(t.id) + '" onclick="generateKey(this.getAttribute(\'data-id\'))">Generate Key</button> ';
    targetHtml += '<button class="settings-btn danger settings-btn-sm" data-id="' + encodeURIComponent(t.id) + '" onclick="deleteTarget(this.getAttribute(\'data-id\'))">Delete</button></div>';
    targetHtml += '</div>';
    targetHtml += '<div class="settings-grid" style="margin-bottom:8px;">';
    targetHtml += '<span class="settings-label">Host</span><span style="color:var(--lcars-text-light);font-size:13px;">' + safeHost + '</span>';
    targetHtml += '</div>';
    targetHtml += '<div class="settings-key-list" id="key-list-' + encodeURIComponent(t.id) + '">';
    if (t.api_keys && t.api_keys.length > 0) {
      var keyParts = new Array(t.api_keys.length);
      for (var j = 0; j < t.api_keys.length; j++) {
        var keyHtml = '<div class="settings-key-item">';
        keyHtml += '<span class="settings-key-value" id="key-val-' + encodeURIComponent(t.id) + '-' + j + '">&#x2022;&#x2022;&#x2022;&#x2022;&#x2022;&#x2022;&#x2022;&#x2022;</span>';
        keyHtml += '<button class="settings-key-copy" title="Copy key" data-id="' + encodeURIComponent(t.id) + '" data-idx="' + j + '" onclick="copyRevealedKey(this.getAttribute(\'data-id\'), parseInt(this.getAttribute(\'data-idx\')))">&#x1F4CB;</button>';
        keyHtml += '<button class="settings-key-copy" title="Delete key" style="color:var(--lcars-red);" data-id="' + encodeURIComponent(t.id) + '" data-idx="' + j + '" onclick="deleteKey(this.getAttribute(\'data-id\'), parseInt(this.getAttribute(\'data-idx\')))">&#x1F5D1;</button>';
        keyHtml += '</div>';
        keyParts[j] = keyHtml;
      }
      targetHtml += keyParts.join('');
      targetHtml += '<button class="settings-btn settings-btn-sm" id="reveal-btn-' + encodeURIComponent(t.id) + '" style="margin-top:4px;background:var(--lcars-blue);font-size:11px;" data-id="' + encodeURIComponent(t.id) + '" onclick="toggleKeys(this.getAttribute(\'data-id\'))">Reveal Keys</button>';
    } else {
      targetHtml += '<div style="color:var(--lcars-tan);font-size:11px;opacity:0.5;">No API keys</div>';
    }
    targetHtml += '</div>';
    targetHtml += '</div>';
    htmlParts[i] = targetHtml;
  }
  container.innerHTML = htmlParts.join('');
}
