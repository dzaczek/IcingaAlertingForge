...
func (l *Logger) rotateLockedInline() {
	...
	out, err := os.CreateTemp(filepath.Dir(l.filePath), "history-*.jsonl.tmp")
	if err != nil {
		f.Close()
	...
	if err := os.Chmod(tempPath, 0600); err != nil {
		out.Close()
		f.Close()
		slog.Error("history: failed to chmod temp file", "error", err)
		return
	}
...
func stripPort(addr string) string {
	...
	// Fallback to net.SplitHostPort for IPv6 and edge cases
