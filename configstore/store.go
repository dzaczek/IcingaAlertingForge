...
func (s *Store) MigrateFromEnv(cfg *config.Config) error {
	...
	for id, t := range cfg.Targets {
		...
		slices.Sort(ts.APIKeys) // ⚡ Bolt: Using slices.Sort() avoids reflection overhead from sort.Strings(), improving performance
...
func (s *Store) ToConfig(serverPort, serverHost string) *config.Config {
	...
	slices.Sort(ids) // ⚡ Bolt: Using slices.Sort() avoids reflection overhead from sort.Strings(), improving performance
	if len(ids) > 0 {
		defaultID = ids[0]
	}
