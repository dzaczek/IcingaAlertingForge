...
function renderTargetCards(targets) {
  ...
  var html = '';
  // Performance optimization: Pre-allocate an array and use a standard for loop to avoid callback overhead and repeated string allocation
  var htmlParts = new Array(targets.length);
  for (var i = 0; i < targets.length; i++) {
    var t = targets[i];
    var safeId = escHtml(t.id || 'unknown');
    var safeHost = escHtml(t.host_name || '-');
    var part = '<div class="settings-target-card">' +
    ...
    if (t.api_keys && t.api_keys.length > 0) {
      ...
      for (var idx = 0; idx < t.api_keys.length; idx++) {
        keyParts[idx] = '<div class="settings-key-item">' +
      ...
      part += '<div style="color:var(--lcars-tan);font-size:11px;opacity:0.5;">No API keys</div>';
    }
    part += '</div></div>';
    htmlParts[i] = part;
  }
