...
## 2026-08-27 - Pre-allocate slice capacity during flattening
**Learning:** When flattening a slice of slices (e.g., `[][]T` into `[]T`), appending dynamically without pre-allocating the destination slice causes repeated underlying array reallocations and memory copying, significantly hurting performance.
**Action:** Always calculate the total length of the nested slices in a first pass, then allocate the destination slice with that exact capacity using `make([]T, 0, totalLength)` before appending the elements.
