...
func (s *Store) MigrateFromEnv(cfg *config.Config) error {
	...
	for id, t := range cfg.Targets {
		...
		// ⚡ Bolt: Using slices.Sort (Go 1.21+) leverages generics and avoids interface boxing/reflection overhead, resulting in faster string slice sorting with zero allocations.
