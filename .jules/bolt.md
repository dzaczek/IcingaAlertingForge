...
## 2026-08-25 - Replace sort.Strings with slices.Sort
**Learning:** The `slices` package (introduced in Go 1.21) provides generic sorting functions like `slices.Sort()` which are significantly faster and produce zero memory allocations compared to the older reflection-based `sort` package methods like `sort.Strings()` or `sort.Slice()`.
**Action:** When sorting slices of primitive types or custom structs in performance-sensitive paths, always prefer the `slices` package over the `sort` package.
