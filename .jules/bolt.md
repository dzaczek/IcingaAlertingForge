...
## 2026-06-29 - Debounce synchronous DOM updates
**Learning:** Performing synchronous DOM updates and layout recalcs directly inside fast-firing UI events like `oninput` blocks the main thread, causing UI stutters.
**Action:** When implementing search or filtering features, always wrap the event handler in a debounce function (e.g., using `setTimeout`) to batch UI operations and ensure smooth interactivity.
