...
## 2026-06-05 - Optimize string construction in webhook hot path
**Learning:** In Go, `fmt.Sprintf` uses reflection to parse format strings and arguments at runtime, which causes unnecessary memory allocations and CPU overhead in hot execution paths (such as parsing incoming high-volume webhooks). Using direct string concatenation (e.g. `+`) and primitive type formatters (like `strconv.FormatInt`) is significantly faster.
**Action:** Replace `fmt.Sprintf` with direct string concatenations and `strconv` calls in all performance-critical hot paths.
